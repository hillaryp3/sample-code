...
#              The sort then takes the remaining portions of the domain and sorts them alphabetically.
...
while ( my $value = <FILE>) {
    ...
    if (my @parts = $domain->match($value)) {
		$tld_to_sort{$parts[0]->{tld}}{"$parts[0]->{hostname}.$parts[0]->{domain}"} = $parts[0]->{match};
	} else {
...
open FILE, ">D:\\Users\\Hillary\\Documents\\sorted_tld_file.txt" or die "couldn't open output file";

# sort the tld portion of the domain
foreach my $tld_key (sort keys %tld_to_sort){
	# sort the remaining portion of the domain
    foreach my $sub_domain_key (sort keys %{$tld_to_sort{$tld_key}}) {
	    # print the sorted line to the file
	    print FILE "$tld_to_sort{$tld_key}{$sub_domain_key}\n";
	}
}
